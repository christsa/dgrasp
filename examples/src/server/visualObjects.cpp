...
//  auto anymalB = server.addVisualArticulatedSystem("v_anymal", binaryPath.getDirectory() + "\\rsc\\mano\\mano.urdf");
  //std::vector<raisim::ArticulatedSystem*> mano;
  world.addArticulatedSystem(binaryPath.getDirectory() + "\\rsc\\mano\\\\mano_conv.urdf");
  //anymalB->color = {0.0,0.0,0.0,0.5};
  auto torso = static_cast<raisim::ArticulatedSystem*>(world.getObject("palm"));
