...
def load_param(weight_path, env, actor, critic, optimizer, data_dir, cfg, store_again=True):
    if weight_path == "":
        raise Exception("\nCan't find the pre-trained weight, please provide a pre-trained weight with --weight switch\n")
    print("\nReloading checkpoint:", weight_path+"\n")
    ...
    if items_to_save is not None and store_again:
