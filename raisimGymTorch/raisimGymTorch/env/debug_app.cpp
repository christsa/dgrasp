...
int main(int argc, char *argv[]) {
  ...
  EigenRowMajorMatDouble observation(config["num_envs"].template As<int>(), vecEnv.getObDim());
  EigenRowMajorMatDouble action(config["num_envs"].template As<int>(), vecEnv.getActionDim());
  EigenVec reward(config["num_envs"].template As<int>(), 1);
  EigenBoolVec dones(config["num_envs"].template As<int>(), 1);
  action.setZero();

  Eigen::Ref<EigenRowMajorMatDouble> ob_ref(observation), action_ref(action);
